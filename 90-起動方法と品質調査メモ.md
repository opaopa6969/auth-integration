...
- ソースは `docker-compose.yml` の `AUTH_PROXY_SRC`(**既定 `../volta-auth-proxy`** = `setup.sh` が
  clone する兄弟 repo)からビルドする。Maven Central の `org.unlaxer:tramli` を解決して fat jar を作る完全自己完結ビルド。
  - ⚠️ **「直したのに変わらない」とき**: 兄弟 `../volta-auth-proxy` が古い checkout のままだと反映されない。
    `setup.sh` は既存 repo も ff pull するよう直したので `./setup.sh` → `docker compose up --build` で更新が乗る
    (キャッシュが怪しければ `--build` 必須、最終手段は `docker compose build --no-cache auth-proxy`)。
...
> 注: `volta-auth-proxy` の修正は remote (`opaopa6969/volta-auth-proxy`) に push 済み。
> compose は兄弟の `../volta-auth-proxy` をビルドするので、**そのローカル checkout を最新に上げないと
> 反映されない**。`./setup.sh` (既存 repo も ff pull する) → `docker compose up --build` で更新が乗る。

---

## 6. 残課題

- **Google ログイン (Part 3)**: GCP で OAuth クライアントを作り、env に `GOOGLE_CLIENT_ID` 等を入れる。
  → 手順は **章 22〜29** に作成済み。設定テンプレは `part3/`、起動は `docker-compose.part3.yml`。
  なお以前は Part 3 のカスタムドメインで **Magic Link のブラウザフォームが CSRF (`isAllowedOrigin`
  ハードコード) で弾かれた**が、`isAllowedOrigin` を `BASE_URL` / `ALLOWED_REDIRECT_DOMAINS`
  対応に修正済 (29章)。Google ログイン自体は元々 GET フローなので影響なし。
