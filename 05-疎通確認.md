...
```bash
./volta-gateway/target/release/examples/mock_auth 7072 &
# => mock auth listening on 127.0.0.1:7072
```

> **後輩**「あれ、設定は 7070 じゃなかったでしたっけ?」

> **先輩**「**本物の volta-auth-proxy がすでに :7070 で動いてた** ので、mock は 7072 に避けた。
...
```
$ ss -tlnp | grep -E ':7070|:7072|:7743|:8888'
*:7070   java (本物の volta-auth-proxy)
:7072    mock_auth (今回の認証 backend)
*:7743   java (todo-sample Jetty)
:8888    volta-gateway
```
...
cd todo-sample
mvn -q jetty:run &
# Jetty が http://localhost:7743 で listen
```
...
# {"message":"listening","addr":"0.0.0.0:8888"}
```

> **先輩**「**:8080 も他のアプリで取られてた** ので :8888 に振った。設定はその通り。」
...
$ curl -s http://localhost:8888/todos
[{"id":2,"title":"gateway経由","done":false,"createdAt":1779663766562}]
```
...
{"state":"ACCESS","method":"POST","host":"localhost","path":"/todos",
 ...
 "user_id":"bench-user-001","upstream":"http://localhost:7743",
...
sequenceDiagram
    ...
    participant A as mock_auth:7072
    participant T as todo-sample:7743
